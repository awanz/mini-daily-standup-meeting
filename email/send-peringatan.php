<?php
  ...
  $email = $_GET['email'];
  $target = $db->getBy("view_user_daily", "email", $email)->fetch_object();
  if (isset($target)) {
    ...
    try {
        ...
        //Content
        $mail->isHTML(true);
        $mail->Subject = 'Peringatan Ketidakaktifan (Magang) - ' . $target->fullname.' ('. $target->email .')';
        $mail->Body    = '
        <p>
        ...
        <p>
            Melalui surat elektronik ini, kami ingin menyampaikan peringatan terkait ketidakaktifan magang berdasarkan daily standup meeting anda hanya mengisi <b><u>'.$target->total_daily.'/15 hari</u></b> (akumulasi).
        </p>
        <p>
            Ketidakaktifan tanpa alasan yang sah dan pemberitahuan terlebih dahulu merupakan pelanggaran disiplin yang serius.
        </p>
        <p>
            Oleh karena itu, kami mohon Anda untuk mengisi daily standup meeting selambat-lambatnya <b>01 '.$nextMonth.'</b>.
        </p>
        <p>
